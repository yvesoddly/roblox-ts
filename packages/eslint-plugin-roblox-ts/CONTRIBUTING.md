...
## Working in the roblox-ts monorepo

See [IMPORT.md](./IMPORT.md) for provenance and package-local validation commands.
The upstream guide below describes the standalone repository. In this monorepo,
Git hooks are not installed automatically, and repository-level CI and release
workflows are not enabled by the copies in this package's `.github/` directory.
